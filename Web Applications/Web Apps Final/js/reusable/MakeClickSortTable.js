...
function MakeClickSortTable(title, objList, sortOrderPropName, sortIcon, link, link2) {
    ...
    function addTableBody(table, list, sortOrderPropName, filterValue) {
        ...
        for (var i in objList) {
            if (isToShow(objList[i], filterValue)) {
                ...
                `;  
                aTag.href = link+objList[i]["id"].innerText;
                
                var del = SortableTableUtils.makeImage("icons/more_icons/delete.png", "1rem");
                const ID = objList[i]["id"].innerText;
                const URL = link2+objList[i]["id"].innerText
                del.onclick = function() {
                    console.log("to delete user " + ID);
                    ...
                    if (confirm("Do you really want to delete user " + ID + "? ")) {
                        ...
                        ajax(URL, DeleteId, document.getElementById("kianError"));                        
                        //location.reload();
                    }
                }
